...
```base3-map
{"title":"German cities","map_type":"street","points":[{"lat":52.520008,"lng":13.404954,"label":"Berlin","description":"**Capital** of Germany"},{"lat":53.551086,"lng":9.993682,"label":"Hamburg","description":"Major port city"},{"lat":48.135124,"lng":11.581981,"label":"Munich","description":"Capital of Bavaria"}]}
```
...
`points` must contain between 1 and 50 objects. Every point requires finite numeric `lat` and `lng` coordinates and a short plain-text `label`. `lat` must be between -90 and 90 and `lng` between -180 and 180. The optional `description` is short Markdown content for the marker popup.
...
Markdown is allowed only inside `description` for paragraphs, emphasis, links, inline code, ordered lists, and unordered lists; encode line breaks there with `\n`. Do not include HTML, JavaScript, callbacks, fenced `base3-*` extension blocks, nested objects, nested arrays, custom marker configuration, or properties other than those documented above and `lat`, `lng`, `label`, plus `description` inside points.
